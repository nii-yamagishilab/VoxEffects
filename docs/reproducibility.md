...
- Degradation modes follow the same public tags as the experiment code: `none`, `pre_only`, `post_only`, `pre_or_post`, and `pre_and_post`.
...
In this release, `mode_tag` matches the degradation mode tag, so deterministic pre/post degradations are also seed-stable when `deterministic_aug=True`.
...
- the degradation config, FFmpeg build, or degradation asset set differs,
- `random_crop=True` is used,
- DataLoader shuffling is enabled for benchmark evaluation.

For benchmark reproduction, use the released protocol CSVs, keep `shuffle=False`, keep `random_crop=False`, keep the same placeholder-root remapping across runs, keep the same degradation mode and `attack.yaml`, and document package versions.
